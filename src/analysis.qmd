...
```{julia}
using XLSX, DataFrames, Statistics, PrettyTables, Statistics, CairoMakie, Survival, HypothesisTests
...
t = deepcopy(model.times)
s = deepcopy(model.survival)
pushfirst!(t, 0)
pushfirst!(s, 1)
...
#| tbl-cap: something.
model = [
    "procedure (surgical resection)" 1.0
    "age" 0.33
    "sex (male)" 0.32
    "smoking" 1.0
    "alcohol" 0.88
    ...
    "T stage (>2)" 0.70
    "N stage (≥2)" 1.0
    "carcinoma ex-pleomorphic adenoma" 0.77
    "perineural invasion" 1.0
    "extracapsular spread" 0.63
    "lymphovascular invasion" 1.0
    "ERBB2 status" 0.47
    "facial nerve sacrifice" 1.0
    ]

pretty_table(
    model; 
    header = ["Covariate", "P value"],
    tf = tf_unicode_rounded)
```
**Table 6**. Covariates associated with recurrence. There were 8 cases for whom recurrence occurred. Continuous covariates were tested using Welch's t-test and categorical covariates were assessed via Fisher's exact test.
